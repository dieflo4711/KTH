...
def computePrior(labels, W=None):
    ...
    # ==========================
    for jdx, classs in enumerate(classes):
        idx = np.where(labels == classs)[0]

        prior[jdx] = idx.shape[0]/Npts

        '''
        wdx = W[idx, :]
        prior[jdx] = np.sum(wdx) / np.sum(W)
        '''
...
def mlParams(X, labels, W=None):
    ...
    for jdx, classs in enumerate(classes):
        ...
        '''
        wlc = W[idx, :]
        mu[jdx] = np.divide(np.dot(wlc.transpose(), xlc), np.sum(wlc, axis=0))
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for k in range(Nclasses):
        for i in range(Npts):
            tmp = (-0.5) * np.log(np.linalg.det(sigma[k]))
            tmp2 = (0.5) * ((X[i, :] - mu[k]) @ np.linalg.inv(sigma[k]) @ (X[i, :] - mu[k]).transpose())
            tmp3 = np.log(prior[k])

            logProb[k][i] =  tmp - tmp2 + tmp3
    # ==========================
...
def trainBoost(base_classifier, X, labels, T=10):
    ...
    for i_iter in range(0, T):
        ...
        factor = np.zeros([Npts, 1])

        for k in range(Npts):
            delta[k] = 1 if vote[k] == labels[k] else 0

        error = np.dot(wCur.transpose(), (1 - delta))
        alpha = 0.5 * (np.log(1 - error) - np.log(error))

        for k in range(Npts):
            factor[k] = np.exp(-alpha) if vote[k] == labels[k] else np.exp(alpha)

        wCur = (wCur * factor) / np.sum(wCur * factor)
